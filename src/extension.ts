...
import ansiRegexGetter from 'ansi-regex';
...
function replaceTerminalColors(text: string) {
	return text
		.replace(errorRegex, (fullMatch: string, inner: string) => {
			return inner.replace(lineRegex, `${chalk.hex("#f38ba8")("$1 $2$3")}`);
		})
		.replace(warnRegex, (fullMatch: string, inner: string) => {
			return inner.replace(lineRegex, `${chalk.hex("#fab387")("$1 $2$3")}`);
		})
		.replace(infoRegex, (fullMatch: string, inner: string) => {
			return inner.replace(lineRegex, `${chalk.hex("#89b4fa")("$1 $2$3")}`);
		});
}
